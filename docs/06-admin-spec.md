...
§10 講的是**視覺語彙**（顏色、字體、青銅只給破壞性與發佈動作），那部分不變。
這一節講的是**資訊架構** —— 選單叫什麼、表格有哪幾欄、表單有哪些欄位、順序如何 ——
第一版是我們自己重新命名重新排的（「文章」「案例」「服務項目」、卡片式列表、
自訂的欄位順序），**業主看了說對不上**。使用這個後台的人熟的是舊系統的用語。

**規則：名稱、欄位與順序一律逐字照 `reference/old/` 的 `.cshtml`。**
要改必須在該處寫下為什麼。

### 選單：直接讀 Lims 表

舊系統的選單是 `Html.SiteMenuAsUnorderedList`（`App_Helpers/HtmlHelperExtensions.cs:185`）
拿 `Lims` 資料渲染的：兩層、`ORDER BY Sort`、網址是 `/backend/{父 Key}/{子 Key}`。

`visibleNav()`（`src/lib/auth/session.ts`）現在做同一件事 —— **不再是寫死的陣列**。
所以選單長這樣：

| 群組 | 項目 |
|---|---|
| 網站管理 | 文章分類維護 · 文章維護 · 服務洽談維護 · 人員維護 · 案例維護 · 關於禾勤 |
| 系統管理 | 管理者維護 |

`Lims` 的 `(父 Key, 子 Key)` 恰好就是 `ROUTE_PERMISSIONS` 的 key，所以權限過濾不用另外對照表。
資料庫加了選單卻沒有對應路由的那一列會被跳過，不會印出一個 404 連結。

### 列表欄位

一律是表格，欄位順序與表頭文字照抄，最後兩欄是「編輯 / 刪除」。

| 頁面 | 欄位（舊系統出處） |
|---|---|
| 文章分類維護 | 排序 · 標題（`WebMs/ArticleTypes.cshtml:43-46`） |
| 文章維護 | 代表圖 · 分類 · 標題 · 日期（`WebMs/Articles.cshtml:51-56`） |
| 服務洽談維護 | 排序 · 代表圖 · 分類 · 標題（`WebMs/Services.cshtml:55-60`） |
| 人員維護 | 排序 · 代表圖 · 姓名 · 標題（`WebMs/Teams.cshtml:55-59`） |
| 案例維護 | 排序 · 分類 · 地點 · 專案 · 標題（`WebMs/Projects.cshtml:55-61`） |
| 管理者維護 | 姓名 · 帳號 · Email（`SettingMs/Admins.cshtml:48-52`） |

三處刻意不同：

1. **人員維護的第三欄**。舊表頭寫「標題」，但那一格印的是 `entity.Name`（`Teams.cshtml:75`）——
   表頭跟內容對不上。這裡照抄它印的東西、把表頭正名為「姓名」，並多印一欄「標題」（職稱）。
2. **「排序」欄的操作方式**。舊系統是每列一個數字輸入框 + 上方「儲存排序」整批送出；
   這裡沿用既有的上/下一格（`/api/admin/{key}/sort`），數字照樣印在中間。
   少一個「改了數字忘了按儲存」的失敗模式。
3. **多一欄「公開網址」**。那是 §10 的簽名元素，也是這個專案存在的理由。

### 表單欄位

| 表單 | 欄位順序（舊系統出處） |
|---|---|
| 文章分類 | 標題 · 子標題 · 簡介 · 背景Class · 代表圖 · 內文（`EditArticleTypes.cshtml:55-106`） |
| 文章 | 文章分類 · 標題 · 代表圖 · 日期 · 內文（`EditArticles.cshtml:55-98`） |
| 服務洽談 | 文章分類 · 標題 · 代表圖（`EditServices.cshtml:55-71`） |
| 人員 | 標題 · 簡介 · 代表圖 · 姓名 · 英文姓名（`EditTeams.cshtml:55-98`） |
| 案例 | 分類 · 地點 · 專案 · 標題（`EditProjects.cshtml:59-101`） |
| 關於禾勤 | 內文 · 代表圖（`Abouts.cshtml:55-69`） |
| 管理者 | 姓名 · 帳號 · 密碼 · Email + 權限矩陣（`EditAdmins.cshtml:57-79`） |

⚠️ **案例的「專案」是 `Title`、「標題」是 `SubTitle`。** 這組命名很反直覺，
但舊後台就是這樣叫的，改掉只會讓熟舊系統的人對不上。

版面也照舊後台的 horizontal form：`<legend>{實體}資料</legend>` + 標籤在左（`.form-row`），
送出鈕文案「確認」。窄螢幕才疊成上下。

⚠️ **「代表圖」現在是 `EntityDef.fields` 的一員**（`kind: 'image'`），
因為它在舊系統的位置是夾在中間的，不是永遠排最後。所以
`src/api/app.ts` 的 `/:entity/save` 迴圈必須 `if (f.kind === 'image') continue` ——
它的值是 `File`，走字串路徑會把 `"[object File]"` 寫進資料庫。
...
Hono 依註冊順序比對。`/:entity/save` 放在 `/Projects/save` 前面，就會把 Projects 的請求也吃掉，然後因為 Projects 不在共用實體層裡而回 **404** —— 安靜地 404，看起來完全像「路由沒建」。

### Projects 的分組是自由文字

`Type` / `Place` / `Title` 直接決定公開頁的分組標題，**打錯一個字就會多出一個只有一筆的分類**。所以表單三個欄位都掛 `datalist` 列出既有的值，列表也印出每一列完整的分組路徑（`分類 › 地點 › 案名`）。

排序完全由 `LegacyOrder` 決定，不是由筆劃。新增的排在最後；刪除留下的號碼缺口沒有影響，它只表達相對先後。

### Admins 的兩道自我保護

- 不能刪除自己的帳號
- 不能在編輯自己時關掉自己的超級使用者旗標

兩個都是把自己鎖在門外的路徑。另外「這是最後一個帳號」也擋掉。

重設別人的密碼一律連帶把 `MustChangePassword` 設回 1 —— 管理者不該知道對方之後在用什麼密碼。
