...
- [x] `smoke:admin` 打已部署的 URL：**48/48**（2026-08-08，重新部署後）。腳本原本把
      `wrangler d1 execute --local` 寫死，加了 `--remote` 才跑得起來。
      重新部署前是 45/48，3 項失敗**全部是部署落後**而非回歸 —— 當時線上跑的是
      2026-08-07 13:12 那一版，斷言卻來自 `28f6515`（列表欄位改版）與
      `0d08fc1`（密碼下限拿掉）。部署 `28f6515` 之後 3 項全部轉綠，證實了這個判讀。
      跑完正式 D1 回原狀：Articles 9 / Projects 87 / Services 0 / Admins 1
