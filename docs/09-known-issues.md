...
**決定變更（2026-08-07 傍晚）：不再原樣保留。**

原本決定照抄，但把 SendGrid key 接上之後，照抄反而**比舊站更糟**：新站有 `await`，所以訪客會**真的**收到一封他們從來沒收過、內容是自己剛填的表單、寄件人是陌生網域的信 —— 而禾勤還是收不到。那是新增的困惑，不是修復。

現在的做法：

| | 值 |
|---|---|
| 收件人 | `CONTACT_TO` secret（可逗號分隔多個） |
